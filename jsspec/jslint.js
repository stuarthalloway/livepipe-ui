...
t=Object.create(t);if(type==='(string)'){if(jx.test(value)){warningAt("Script URL.",line,from);}}else if(type==='(identifier)'){t.identifier=true;if(option.nomen&&(value.charAt(0)==='_'||value.charAt(value.length-1)==='_')){warningAt("Unexpected '_' in '{a}'.",line,from,value);}}
...
break klass;case'\\':c=s.charAt(0);if(c<' '){warningAt("Unexpected control character in regular expression.",line,from+l);}else if(c==='<'){warningAt("Unexpected escaped character '{a}' in regular expression.",line,from+l,c);}
...
function substyle(){for(;;){if(nexttoken.id==='}'||xquote&&nexttoken.id===xquote){return;}
if(nexttoken.id==='*'){advance('*');}
if(!nexttoken.identifier){error("Excepted a style attribute, and instead saw '{a}'.",nexttoken,nexttoken.value);}
advance();advance(':');for(;;){if(nexttoken.id===';'||nexttoken.id===':'){break;}
if(nexttoken.id==='(end)'||nexttoken.id==='}'||nexttoken.id==='{'){return;}
if(option.adsafe&&nexttoken.value.toLowerCase()==='url'){warning("ADsafe style url violation.");}
advance();}
advance(';');}}
function stylePattern(){if(nexttoken.id==='{'){warning("Expected a style pattern, and instead saw '{a}'.",nexttoken,nexttoken.id);}
while(nexttoken.id!=='</'&&nexttoken.id!=='{'&&nexttoken.id!==';'&&nexttoken.id!=='(end)'){advance();}}
